...
class Executor(base_executor.BaseExecutor):
  ...
  def Do(self, input_dict: Dict[Text, List[types.Artifact]],
    ...
    with self._InstallGracefulShutdown():
      self._Do(
          model=model,
          examples=examples,
          blessing=blessing,
  ...
  def _InstallGracefulShutdown(self):
    ...
    signal the gracefully shutdown. Python default behavior for receiving
    SIGTERM is to terminate the process without raising any exception. By
    registering a handler that raises on signal, we can effectively transform
    the signal to an exception, and we can reuse our cleanup code inside
    "except" or "finally" block during the grace period.
    ...
    try:
      ...
      logging.warning('Unable to register signal handler for non-MainThread '
